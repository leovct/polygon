...
contract Snowball {
    ...
    function test(uint64 _seed, uint32 _loops, Mode _mode) public payable returns (bytes32) {
        ...
        for (uint32 i = 0; i < _loops; i = i + 1) {
            ...
            if (_mode == Mode.Contract) {
                ...
                (success, returnData) =
                    address(counter).delegatecall{gas: 50000}(abi.encodeWithSignature("increment()"));
                snowball = keccak256(abi.encodePacked(snowball, success, returnData));
            ...
            if (_mode == Mode.Precompile) {
                ...
                d = generateNumber();
                snowball =
                    keccak256(abi.encodePacked(snowball, callBn256Add(bytes32(a), bytes32(b), bytes32(c), bytes32(d))));
    ...
    function callDatacopy(bytes memory data) public returns (bytes memory) {
        ...
        assembly {
            let len := mload(data)
            if iszero(call(gas(), 0x04, 0, add(data, 0x20), len, add(result, 0x20), len)) { invalid() }
        }
        return result;
    }
}
