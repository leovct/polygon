...
contract LoadTester {
  ...
  function testNOT(uint x) public returns (uint) {
    ...
    assembly {
      ...
      if iszero(eq(result, 0xDEADBEEF0019)) {
        result := not(result)
      }
    }
    return result;
  ...
  function testSHA3(uint x) public returns (uint) {
    ...
    assembly {
      ...
      if iszero(
        eq(
          out,
          hex'29045a592007d0c246ef02c2223570da9522d0cf0f73282c79a1bc8f0bb2c238'
        )
      ) {
        result := 0
      }
  ...
  function testECRecover(
    ...
    assembly {
      ...
      let success := call(
        gas(),
        EC_RECOVER_PRECOMPILED_CONTRACT,
        0,
        inputPtr,
        0x80,
        mload(0x40),
  ...
  function testRipemd160(
    ...
    assembly {
      ...
      let success := call(
        gas(),
        RIPEMD160_PRECOMPILED_CONTRACT,
        0,
        inputPtr,
        inputLength,
        outputPtr,
